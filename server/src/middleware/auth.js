...
export const validateAuthentication = async (req, res, next) => {
  ...
  try {
    ...
    const internalError = new NoValidationEvent('Invalid access token');
    myEmitterErrors.emit('error', internalError);
    return sendMessageResponse(res, internalError.code, {
      authorization: internalError.message,
    });
  }
